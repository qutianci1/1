...
-- @Last Modified time  : 2023-05-06 06:35:36
local 法术 = {
    类别 = '坐骑',
    类型 = 2,
    对象 = 0,
...
function 法术:效果计算(坐骑)
    ...
    local mp = (根骨 * 坐骑系数[5] + 灵性 * 坐骑系数[4] + 力量 * 坐骑系数[1]) * 成长 / 坐骑参数[14][1] + 熟练 / 坐骑系数[7] / 坐骑参数[14][2]
    local qlh = (根骨 * 坐骑系数[6] + 灵性 * 坐骑系数[4] + 力量 * 坐骑系数[1]) * 成长 / 坐骑参数[15][1] + 熟练 / 坐骑系数[7] / 坐骑参数[15][2]
    local klh = (根骨 * 坐骑系数[5] + 灵性 * 坐骑系数[3] + 力量 * 坐骑系数[2]) * 成长 / 坐骑参数[16][1] + 熟练 / 坐骑系数[7] / 坐骑参数[16][2]

    mp = math.ceil(mp * 100) / 100
    qlh = math.ceil(qlh * 100) / 100
    klh = math.ceil(klh * 100) / 100
    return mp, qlh, klh
end

function 法术:取描述(坐骑)
    local mp, qlh, klh = self:效果计算(坐骑)
    return string.format("#G增加MP#R%s%%#G,加强雷、火、鬼火#R%s%%#G,增加抗雷、火、鬼火#R%s%%#G,", mp, qlh
        , klh)
end
