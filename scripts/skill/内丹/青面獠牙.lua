...
-- @Last Modified time  : 2023-08-23 23:26:19
local 法术 = {
    类别 = '内丹',
    类型 = 1,
    对象 = 2,
...
function 法术:取伤害(召唤)
    ...
    local 数值=math.floor((((召唤.等级*self.等级/698000)^(1/2))*(1+0.25*self.转生)+(亲密^(1/6))*self.等级/7500)*1000)/1000+0.01
    local 系数=0
    if 数值>0.999 then
      系数=0.999
    else
...
function 法术:法术取描述(召唤)
   local 消耗mp,伤害比 = self:取伤害(召唤)
    return string.format("通过牺牲自己MP当前的#R%.2f%%#W给对手造成自己MP当前值#R%.2f%%#W的HP伤害。"
        , 消耗mp, 伤害比)
end
