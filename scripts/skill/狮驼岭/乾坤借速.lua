...
-- @Last Modified time  : 2023-09-03 14:26:27
...
function 法术:法术取BUFF效果(攻击方, 挨打方)
    ...
    elseif self.熟练度>=20000 then
        速度加成=0.199+(self.熟练度-20000)*0.00014/100
    elseif self.熟练度>=15000 then
        速度加成=0.191+(self.熟练度-15000)*0.00016/100
    elseif self.熟练度>=10000 then
        速度加成=0.18+(self.熟练度-10000)*0.00022/100
    elseif self.熟练度>=5000 then
        速度加成=0.164+(self.熟练度-5000)*0.00032/100
    else
        速度加成=0.08+self.熟练度*0.00168/100
    end
...
function 法术:法术取描述()
    ...
    速度加成=速度加成*100
    return string.format('增加#RSP%s#G，使用效果#R%s#G人，持续效果#R%s#G个回合。', 速度加成..'%',self:法术取回合(),self:法术取回合())
end
