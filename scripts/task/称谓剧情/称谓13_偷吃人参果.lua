...
--17
local _详情 = {
    '你可以完成第十三个称谓剧情任务了，任务领取人#Y五庄观#G明月',
    '要了解人参果的秘密，去找#G袁天罡#W想想办法。',
    '去大唐边境捕捉一只毒蜂后再来找#G袁天罡#W，袁天罡会把你变成毒蜂的那个样子，这样你就可以偷听人参果的秘密了。',
    '变成蜜蜂，去偷听两个道童#G清风#W和#G明月#W说话。',
    '去找#G南天王#W要瞌睡虫。',
    '快去找南天王的化身--#G王五',
    '到天宫找#G南天王#W要瞌睡虫。',
    '瞌睡虫到手了，金击子和丹盘有着落了。#Y（找到两个道童#G清风#Y和#G明月，#Y分别对他们使用瞌睡虫，#R ALT+A给予瞌睡虫给他们。#Y）',
    '得到金击子和丹盘了，去找到#G人参果树#W拿人参果吧。',
    '奇怪，人参果怎么不见了，问问这里的#G人参果园土地#W。',
    '再找到#G人参果树#W试试拿次人参果。',
    '人参果到手了，嘿嘿，把金击子和丹盘还给#G清风#W和#G明月#W吧。#Y( ALT+G将金击子和丹盘分别给子清风、明月)',
    '可恶，两个小道重居然恶语相向，好，推倒#G人参果树#W，大家一拍两散。#R（与人参果树对话结束后直接进入战斗！）',
    '去问问#G袁天罡#W有没什么办法医治人参果树。',
    '去找#G灵兽仙#W打听下医治人参果树的法子。',
    '去看看#G观音姐姐#W有没办法医治人参果树。',
    '终于可以医治#G人参果树#W了，赶紧回五庄观吧。#Y(ALT+G将净瓶给予人参果树) ',
    '人参果树已经医好了，去找#G镇元大仙#W聊聊。'
}
...
local _台词 = {
    ...
    [15] = {头像 = 3056, 结束 = false, 台词 = '嗯，好，我在你身上施下幻化之术，你就以蜜蜂之形去五庄观吧，呵呵，不过这幻化之术在你听完他们谈话之后就会自动除去，将来地仙问起来，我可不知道呦。'},
    [16] = {头像 = 0, 台词 = '这老袁，这么怕事....'},
    --2
    ...
    [57] = {头像 = 3107, 结束 = false, 台词 = '好，再试试看，小心用丹盘接着，我敲....'},
    ...
    [67] = {头像 = 0, 结束 = false, 台词 = '(可恶，这两个小道审真是可恨，拿两个果子吃，居然敢这样出言辱骂，好！我去推倒那破树，大家一拍两散)'},
    --12

    [68] = {头像 = 0, 结束 = false, 台词 = '哼，就是这个破人参果树，居然害得我这么辛苦。好，两个小道童是吧，我就把这树推倒，让你们知道我的手段。'},
    [69] = {头像 = 0, 结束 = false, 台词 = '嗯?这树颇有灵气，似乎已经修成了通灵之身。哼，区区树灵，我还不放在眼里! !'},
    --战斗
    ...
    [77] = {头像 = 0, 结束 = false, 台词 = '哼哼，镇元大仙，你法力通天，他们怕你，我可不怕，就和你比比看，到底是谁法力高强。'},
    --战斗
    ...
    [86] = {头像 = 3056, 结束 = false, 台词 = '啊?你这. ..早知道就不该帮你。洛阳的灵兽仙好象统管天下生灵，人参果虽是仙木，到底也还是万物生灵，你去问问他有没法子。他如果还没法子，我也不知道了。'},
    --14
...
function 任务:任务NPC对话(玩家, NPC)
    ...
    elseif NPC.名称 == '清风' and NPC.台词 then
        if self.进度 == 3 then
            self.对话进度 = 17
            NPC.台词, NPC.头像, NPC.结束 = self:取对话(玩家)
        end
    ...
    elseif NPC.名称 == '南天王' and NPC.台词 then
        ...
        elseif self.进度 == 6 then
            ...
            NPC.台词, NPC.头像, NPC.结束 = self:取对话(玩家)
            self.进度 = 7
            玩家:添加物品({生成物品 {名称 = '瞌睡虫', 数量 = 2}})
        end
    elseif NPC.名称 == '王五' and NPC.台词 then
        if self.进度 == 5 then
            self.对话进度 = 41
            NPC.台词, NPC.头像, NPC.结束 = self:取对话(玩家)
        end
    elseif NPC.名称 == '人参果树' and NPC.台词 then
        if self.进度 == 8 then
            self.对话进度 = 48
            NPC.台词, NPC.头像, NPC.结束 = self:取对话(玩家)
    ...
    elseif NPC.名称 == '人参果园土地' and NPC.台词 then
        if self.进度 == 9 then
            self.对话进度 = 51
            NPC.台词, NPC.头像, NPC.结束 = self:取对话(玩家)
        end
    elseif NPC.名称 == '灵兽仙' and NPC.台词 then
        if self.进度 == 14 then
            self.对话进度 = 87
            NPC.台词, NPC.头像, NPC.结束 = self:取对话(玩家)
        end
    ...
    elseif NPC.名称 == '镇元大仙' and NPC.台词 then
        if self.进度 == 17 then
            self.对话进度 = 99
            NPC.台词, NPC.头像, NPC.结束 = self:取对话(玩家)
        end
