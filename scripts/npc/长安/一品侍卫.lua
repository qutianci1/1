...
local 对话 = {
    [[
奉唐王旨意,征召雇佣兵外出杀敌，凡有心效力朝廷,外出奋勇杀敌均有双倍经验奖励。你是否愿意外出杀敌?(你当前未领取的双倍时间还有%s小时)
menu
1|领取一小时
2|领取两小时
3|领取四小时
...
function NPC:NPC对话(玩家)
    local r = 玩家:取任务('双倍时间')
    if r and r.冻结 then
        return string.format(对话[3], r.剩余时间 // 60, 660 - 玩家:取双倍时间数据())
    end
    return string.format(对话[1], 660 - 玩家:取双倍时间数据())
end
--
