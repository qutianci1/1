...
function map:处理事件(tb)
    ...
    if hour >= 20 and hour <= 22 then --这个10是 控制 多久给一次的   我想设定一个时间段能实现吗 这个地图的
        if minute == 0 and  second == 0 then
            tb.接口:发送系统('#Y【天女散花】每晚8点~10点！开启天女散花活动，玩家在长安城地图内，可获得天女散花奖励，奖励丰厚#56')
        end
        if os.time() - 倒计时 >= 600 then --600
            tb.接口:发送系统('#Y【天女散花】每晚8点~10点！开启天女散花活动，玩家在长安城地图内，可获得天女散花奖励，奖励丰厚#56')  --这样就完了 自己耍吧
            倒计时 = os.time()
